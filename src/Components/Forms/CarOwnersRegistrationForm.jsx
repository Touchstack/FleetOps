...
import { apiRegisterCarOnwer } from "../../services/VehiclesService";
...
const CarOwnersRegistrationForm = () => {
  ...
  const registerCarOwner = async (values) => {
    try {
      setError(false);
      setLoading(true);
      await apiRegisterCarOnwer(values);
      setLoading(false);
      return (window.location.href = "/carowners/registration/success");
    } catch (error) {
      setLoading(false);
      setError(true);
  ...
  return (
    <div className="py-24 flex flex-col justify-center items-center">
      ...
      <Formik
        ...
        onSubmit={(values) => registerCarOwner(values)}
      >
